...
var TextEditorTool = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(TextEditorTool, [{
    ...
    value: function _parseMarkdownToHTML(markdown) {
      // Convert headings
      var html = markdown.replace(/^### (.*$)/gim, '<h3 style="font-weight: 600; margin: 0;">$1</h3>'); // Convert bold text

      html = html.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>'); // Convert italic text

      html = html.replace(/\*(.*?)\*/g, '<em>$1</em>'); // Convert new lines
